...
class Tilt_solar_angle(Solar_angle):
    ...
    def return_azi_zeni_angle(self):
        vector_x = np.tan(self.zenith_angle_deg) * np.sin(self.azimuth_angle_deg)
        vector_y = -np.tan(self.zenith_angle_deg) * np.cos(self.azimuth_angle_deg)
        vector_z = -1
        return vector_x, vector_y, vector_z


if __name__ == "__main__":
    a = Tilt_solar_angle(year=2017, month=6, day=15, standard_time=3, standard_longitude=15, local_longitude=5.49, local_latitude=55, tilt_angle=30)
    b = Solar_angle(year=2017, month=6, day=15, standard_time=3, standard_longitude=15, local_longitude=5.49, local_latitude=55)
    a.run_solar()
    b.run_solar()
    print(a.day_length_hour, b.day_length_hour)
