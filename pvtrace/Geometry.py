...
def cmp_floats(a, b):
    """
    Returns true if the difference between the two values is smaller than 1 pm (1e-12 m)
...
def cmp_floats_range(a, b):
    """
    Compare two floats and returns
    * -1 if a < b
...
def interval_check(a, b, c, strict=False):
    """
...
def closest_point(reference, point_list):
    ...
    :param reference: Point to measure the distance from
    :param point_list: List of points whose distance to reference is to be evaluated
    """
    separations = []
...
def transform_point(point, transform):
    # return equals
    # np.array(np.dot(transform, np.matrix(np.concatenate((point, [1.]))).transpose()).transpose()[0,0:3]).squeeze()
...
class Plane(object):
    ...
    def surface_identifier(surface_point, assert_on_surface=True):
        raise Exception('planar_surface')
...
class FinitePlane(Plane):
    ...
    def intersection(self, ray):
        """
        Returns a intersection point with a ray and the finite plane.
...
class Polygon(object):
    ...
    def on_surface(self, point):
        ...
        for i in range(n):
            ...
            angle_sum = angle_sum + np.arccos(cos_theta)
        return cmp_floats(angle_sum, 2 * np.pi)
    ...
    def intersection(self, ray):
        ...
        t = 1 / (np.dot(np.array(ray.direction), n)) * (
            np.dot(n, np.array(self.pts[0])) - np.dot(n, np.array(ray.position)))
...
class Cylinder(object):
    """
    Parametrised standard representation of a cylinder. The axis is aligned along z but the radius
...
class Sphere(object):
    ...
    def surface_identifier(surface_point, assert_on_surface=True):
        """
        Returns the surface of the sphere. One surface, static method.
    ...
    def contains(self, point):
        ...
        False
        """
        point_radius_sq = (point[0] - self.centre[0]) ** 2 + (point[1] - self.centre[1]) ** 2 + (point[2] - self.centre[
            2]) ** 2
