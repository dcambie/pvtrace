...
def autorunning_func(LSC_start_time=LSC_start_time):
#1 Create LSC-PM DYE material and host material
    while (LSC_start_time<=LSC_finish_time):
        ...
        trace = pvtrace.Tracer(scene=scene, source=lamp.source, throws=10, use_visualiser=True,
        ...
        yield close_me.run_close()
        LSC_start_time += 0.3
    # LSC_start_time += 0.2

for i in autorunning_func():
    frame = cv2.imread(i)
