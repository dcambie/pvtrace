...
source = PlanarSource(direction=(0, 0, -1), spectrum=oriel, length=config['L'],
                      width=config['W'])  # Incident light (perpendicular to device)
...
for i in range(1, 2):
    ...
    ems = Spectrum([0, 1000],
                   [0.1, 0])  # Giving emission suppress error. It's btw not used due to quantum_efficiency=0 :)
    ...
    trace = Tracer(scene=scene, source=source, seed=random_seed, throws=config['photons_to_throw'],
                   database_file=dbfile, use_visualiser=config['visualizer'], show_log=config['debug'], show_axis=True)
    ...
    luminescent_edges = len(trace.database.uids_out_bound_on_surface('left', luminescent=True)) + len(
        trace.database.uids_out_bound_on_surface('right', luminescent=True)) + len(
        trace.database.uids_out_bound_on_surface('near', luminescent=True)) + len(
        trace.database.uids_out_bound_on_surface('far',
                                                 luminescent=True))  # Photons on edges are only luminescent with planarsource perpendicular to device, so luminescent=true not really needed in *THIS* case
    luminescent_apertures = len(trace.database.uids_out_bound_on_surface('top', luminescent=True)) + len(
        trace.database.uids_out_bound_on_surface('bottom', luminescent=True))
    ...
    if config['informative_output']:
        ...
        for surface in edges:
            print "\t", surface, "\t", len(
                trace.database.uids_out_bound_on_surface(surface, luminescent=True)) / thrown * 100, "%"
        ...
        for surface in edges:
            print "\t", surface, "\t", len(
                trace.database.uids_out_bound_on_surface(surface, solar=True)) / thrown * 100, "%"
